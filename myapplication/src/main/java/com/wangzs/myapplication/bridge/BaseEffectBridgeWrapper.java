package com.wangzs.myapplication.bridge;
...
import com.wangzs.myapplication.view.MainUpView;

/**
 * 动画效果基类，不对外使用，提供其他几种动画效果基类继承
 * 未使用动画监听，边框绘制，只提供方法实现
 *
 */
public class BaseEffectBridgeWrapper extends BaseEffectBridge {

    private static final float DEFAULT_SCALE = 1.0f;
    private static final int DEFAULT_TRAN_DUR_ANIM = 250;
    ...
    public void onDrawShadow(Canvas canvas) {
        ...
        if (drawableShadow != null) {
            ...
            drawableShadow.setBounds(-padding.left - (int) Math.rint(shadowPaddingRect.left), -padding.top - (int) Math.rint(shadowPaddingRect.top),
                    width + padding.right + (int) Math.rint(shadowPaddingRect.right), height + padding.bottom + (int) Math.rint(shadowPaddingRect.bottom));
    ...
    public void onDrawUpRect(Canvas canvas) {
        ...
        if (drawableUp != null) {
            ...
            drawableUp.setBounds(-padding.left - (int) Math.rint(paddingRect.left), -padding.top - (int) Math.rint(paddingRect.top),
                    width + padding.right + (int) Math.rint(paddingRect.right), height + padding.bottom + (int) Math.rint(paddingRect.bottom));
