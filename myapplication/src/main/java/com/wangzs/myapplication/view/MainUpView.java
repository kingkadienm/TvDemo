 ...
 */
package com.wangzs.myapplication.view;
...
import com.wangzs.myapplication.bridge.BaseEffectBridge;
import com.wangzs.myapplication.bridge.OpenEffectBridge;
 ...
 * <li>{@link #setDrawUpRectPadding(RectF)} 用于调整阴影图片边距.
 * <p>
 * Effcet相关API:
...
public class MainUpView extends FrameLayout {
     ...
     * @param scale   放大的比例, 可以设置为1.2f, 1.1f或者其它
     */
    public void setFocusViewWhenBorder(View newView, View oldView, float scale) {
        setFocusView(newView, scale);
        if (newView == oldView) {
            if (!newView.hasFocus()) {
                setUnFocusView(oldView);
